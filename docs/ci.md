...
| Quality | Root/native fmt, strict all-feature Clippy, deployment/script Python tests, actionlint | Static/build/test-driver checks |
| GTK | Three exact ignored tests under separate Xvfb/D-Bus sessions | QR expiry, Create validation and thumbnail lifetime; not a performance benchmark |
...
- Future signing must use a separately reviewed trusted workflow and protected
  environment; ordinary PR jobs must not receive those secrets.
...
The 17 existing ignored Rust tests are deliberately not passed to a blanket
`--include-ignored`. Run each GTK test in its own process; a single test thread
...
| GTK CI subset (3) | QR expiry, Create validation, preview replacement | Exact allowlist in CI, one process per test |
| Additional GTK UI (3) | About artwork; album virtualization/viewer/performance; album preview lifecycle faults | Dedicated graphical acceptance; hardware-dependent metrics must stay separate |
